...
#define SIM7000_GNSS_DATA_UTC_LEN 20
/** Maximum SMS length */
#define SIM7000_SMS_MAX_LEN 160
/** Maximum UE system information band size */
#define SIM7000_UE_SYS_INFO_BAND_SIZE 32
...
enum sim7000_state {
	...
	SIM7000_STATE_GNSS, /**< GNSS active */
	SIM7000_STATE_OFF, /**< Modem off */
};

/** Sim7000 gnss data structure */
struct sim7000_gnss_data {
	/**
	 ...
	 */
	char utc[SIM7000_GNSS_DATA_UTC_LEN];
	/**
...
enum sim7000_sms_stat {
	SIM7000_SMS_STAT_REC_UNREAD = 0, /**< Message unread */
	SIM7000_SMS_STAT_REC_READ, /**< Message read*/
	SIM7000_SMS_STAT_STO_UNSENT, /**< Message stored unsent */
	SIM7000_SMS_STAT_STO_SENT, /**< Message stored sent */
	SIM7000_SMS_STAT_ALL, /**< Status count */
};

/** Possible ftp return codes. */
enum sim7000_ftp_rc {
	SIM7000_FTP_RC_OK = 0, /**< Operation finished correctly. */
	SIM7000_FTP_RC_FINISHED, /**< Session finished. */
	SIM7000_FTP_RC_ERROR, /**< An error occurred. */
};
...
struct sim7000_sms {
	...
	/** Buffered sms. */
	char data[SIM7000_SMS_MAX_LEN + 1];
 ...
 */
struct sim7000_sms_buffer {
	/** sms structures to read to. */
	struct sim7000_sms *sms;
	/** Number of sms structures. */
	uint8_t nsms;
};

/** UE system mode */
enum sim7000_ue_sys_mode {
	SIM7000_UE_SYS_MODE_NO_SERVICE, /**< No service */
	SIM7000_UE_SYS_MODE_GSM, /**< GSM */
	SIM7000_UE_SYS_MODE_LTE_CAT_M1, /**< LTE CAT M1 */
	SIM7000_UE_SYS_MODE_LTE_NB_IOT, /**< LTE NB IOT */
};

/** UE operating mode */
enum sim7000_ue_op_mode {
	SIM7000_UE_OP_MODE_ONLINE, /**< Online */
	SIM7000_UE_OP_MODE_OFFLINE, /**< Offline */
	SIM7000_UE_OP_MODE_FACTORY_TEST_MODE, /**< Factory test mode */
	SIM7000_UE_OP_MODE_RESET, /**< Reset */
	SIM7000_UE_OP_MODE_LOW_POWER_MODE, /**< Low power mode */
};

/**
 * Sim7000 ue system information structure for gsm.
 */
struct sim7000_ue_sys_info_gsm {
	/** Mobile country code */
	uint16_t mcc;
	/** Mobile network code */
	uint16_t mcn;
	...
	uint8_t arfcn[SIM7000_UE_SYS_INFO_BAND_SIZE + 1];
	/** RX level in dBm */
	int16_t rx_lvl;
	/** Track LO adjust */
...
/**
 * Sim7000 ue system information structure for LTE.
 */
struct sim7000_ue_sys_info_lte {
	/** Mobile country code */
	uint16_t mcc;
	/** Mobile network code */
	uint16_t mcn;
	...
	uint16_t pci;
	/** Frequency band */
	uint8_t band[SIM7000_UE_SYS_INFO_BAND_SIZE + 1];
...
/**
 * Sim7000 ue system information structure.
 */
struct sim7000_ue_sys_info {
	/** Refer to sim7000_ue_sys_mode */
	enum sim7000_ue_sys_mode sys_mode;
	/** Refer to sim7000_ue_op_mode */
	enum sim7000_ue_op_mode op_mode;
	union {
		/** Only set if sys_mode is GSM */
		struct sim7000_ue_sys_info_gsm gsm;
		/** Only set if sys mode is LTE CAT-M1/NB-IOT */
		struct sim7000_ue_sys_info_lte lte;
 ...
 * Concurrent use of network and gnss is not possible.
 */
int mdm_sim7000_start_gnss(void);
 ...
 * @warning On some modem firmware revisions (e.g. 1951B16SIM7000) power cycling the
 *          gnss functionality with an xtra file can cause a hangup of the modem.
 *          This can be triggered by calling mdm_sim7000_start_gnss_xtra(),
 *          then turning off gnss by calling mdm_sim7000_stop_gnss() and then re-enabling gnss
 *          by calling mdm_sim7000_start_gnss_xtra() again. In this case the modem will not
 *          respond to commands anymore and every call of a modem function will result in a timeout.
 *          In this case the whole modem needs to be power cycled by calling mdm_sim7000_power_off()
 *          and mdm_sim7000_power_on(). It is advised to power cycle the modem before using the gnss
 *          functionality to avoid hangups.
 */
int mdm_sim7000_start_gnss_xtra(void);
 ...
 * @return 0 on success. Otherwise <0 is returned.
 */
int mdm_sim7000_stop_gnss(void);
 ...
 * @return 0 on success. Otherwise -1 is returned.
 */
int mdm_sim7000_delete_sms(uint16_t index);
 ...
 */
int mdm_sim7000_set_gpio(int gpio, int level);
 ...
 */
int mdm_sim7000_ftp_get_start(const char *server, const char *user, const char *passwd,
 ...
 * @return According sim7000_ftp_rc.
 */
int mdm_sim7000_ftp_get_read(char *dst, size_t *size);
 ...
 */
int mdm_sim7000_get_battery_charge(uint8_t *bcs, uint8_t *bcl, uint16_t *voltage);
 ...
 * @return 0 on success. Otherwise a negative error is returned.
 */
int mdm_sim7000_get_ue_sys_info(struct sim7000_ue_sys_info *info);
 ...
 */
int mdm_sim7000_get_local_time(struct tm *t);
