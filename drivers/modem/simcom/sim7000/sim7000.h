...
#ifndef SIMCOM_SIM7000_H
#define SIMCOM_SIM7000_H
...
#define MDM_APN CONFIG_MODEM_SIMCOM_SIM7000_APN
#define MDM_LTE_BANDS CONFIG_MODEM_SIMCOM_SIM7000_LTE_BANDS
...
enum sim7000_ftp_connection_state {
	/* Not connected yet. */
	SIM7000_FTP_CONNECTION_STATE_INITIAL = 0,
	/* Connected and still data available. */
	SIM7000_FTP_CONNECTION_STATE_CONNECTED,
	/* All data transferred. */
	SIM7000_FTP_CONNECTION_STATE_FINISHED,
	/* Something went wrong. */
	SIM7000_FTP_CONNECTION_STATE_ERROR,
};

enum sim7000_status_flags {
	SIM7000_STATUS_FLAG_POWER_ON = 0x01,
	SIM7000_STATUS_FLAG_CPIN_READY = 0x02,
	SIM7000_STATUS_FLAG_ATTACHED = 0x04,
	SIM7000_STATUS_FLAG_PDP_ACTIVE = 0x08,
};
...
struct sim7000_data {
	...
	struct sim7000_sms_buffer *sms_buffer;
	...
	struct {
		...
		enum sim7000_ftp_connection_state state;
	} ftp;
	/*
	 * Semaphore(s).
	 */
 ...
 * Driver internals
 */
extern struct sim7000_data mdata;
...
extern void sim7000_rssi_query_work(struct k_work *work);

enum sim7000_state sim7000_get_state(void);

void sim7000_change_state(enum sim7000_state state);

int sim7000_pdp_activate(void);
int sim7000_pdp_deactivate(void);

int sim7000_offload_socket(int family, int type, int proto);

void sim7000_handle_sock_data_indication(int fd);

void sim7000_handle_sock_state(int fd, uint8_t state);

int sim7000_utils_parse_time(uint8_t *date, uint8_t *time_str, struct tm *t);

#endif /* SIMCOM_SIM7000_H */
