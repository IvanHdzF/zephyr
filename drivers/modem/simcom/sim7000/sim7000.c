...
#define DT_DRV_COMPAT simcom_sim7000

#include <zephyr/logging/log.h>
#include <zephyr/net/offloaded_netdev.h>
LOG_MODULE_REGISTER(modem_simcom_sim7000, CONFIG_MODEM_LOG_LEVEL);
...
static K_KERNEL_STACK_DEFINE(modem_rx_stack, CONFIG_MODEM_SIMCOM_SIM7000_RX_STACK_SIZE);
static K_KERNEL_STACK_DEFINE(modem_workq_stack, CONFIG_MODEM_SIMCOM_SIM7000_RX_WORKQ_STACK_SIZE);
...
{
	struct sim7000_data *data = dev->data;
	uint32_t hash_value;
...
static void modem_net_iface_init(struct net_if *iface)
{
	...
	net_if_socket_offload_set(iface, sim7000_offload_socket);
}

/**
 * Changes the operating state of the sim7000.
 *
 * @param state The new state.
 */
void sim7000_change_state(enum sim7000_state state)
{
...
/**
 * Get the current operating state of the sim7000.
 *
 * @return The current state.
 */
enum sim7000_state sim7000_get_state(void)
{
	return mdata.state;
}
...
static bool offload_is_supported(int family, int type, int proto)
{
	if (family != NET_AF_INET &&
	    family != NET_AF_INET6) {
		LOG_WRN("Offload is not supported! family is: %d", family);
		return false;
	}

	if (type != NET_SOCK_DGRAM &&
	    type != NET_SOCK_STREAM) {
		LOG_WRN("Offload is not supported! type is: %d", type);
		return false;
	}

	if (proto != NET_IPPROTO_TCP &&
	    proto != NET_IPPROTO_UDP) {
		LOG_WRN("Offload is not supported! proto is: %d", proto);
		return false;
	}
...
{
	bool active = strcmp(argv[0], "ACTIVE") == 0;

	if (active) {
		mdata.status_flags |= SIM7000_STATUS_FLAG_PDP_ACTIVE;
	} else {
		mdata.status_flags &= ~SIM7000_STATUS_FLAG_PDP_ACTIVE;
	}
...
MODEM_CMD_DEFINE(on_urc_cadataind)
{
	...
	sim7000_handle_sock_data_indication(sock_fd);
	return 0;
}
...
MODEM_CMD_DEFINE(on_urc_castate)
{
	...
	sim7000_handle_sock_state(sockfd, state);
	return 0;
}
...
MODEM_CMD_DEFINE(on_urc_ftpget)
{
	...
	/* Transfer finished. */
	if (error == 0) {
		mdata.ftp.state = SIM7000_FTP_CONNECTION_STATE_FINISHED;
	} else if (error == 1) {
		mdata.ftp.state = SIM7000_FTP_CONNECTION_STATE_CONNECTED;
	} else {
		mdata.ftp.state = SIM7000_FTP_CONNECTION_STATE_ERROR;
	}
...
/*
 * Possible responses by the sim7000.
 */
...
static const struct modem_cmd unsolicited_cmds[] = {
	MODEM_CMD("+APP PDP: ", on_urc_app_pdp, 1U, ","),
...
int modem_autobaud(void)
{
	...
	/*
	 * The sim7000 has a autobaud function.
...
static int modem_boot(bool allow_autobaud)
{
	...
	while (boot_tries++ <= MDM_BOOT_TRIES) {
		...
		if (ret == 0) {
			if (mdata.status_flags & SIM7000_STATUS_FLAG_POWER_ON) {
		...
		ret = modem_set_baudrate(CONFIG_MODEM_SIMCOM_SIM7000_BAUDRATE);
		if (ret != 0) {
		...
		if ((mdata.status_flags & SIM7000_STATUS_FLAG_POWER_ON) == 0) {
	...
	sim7000_change_state(SIM7000_STATE_IDLE);
...
#if IS_ENABLED(CONFIG_MODEM_SIMCOM_SIM7000_BOOT_TYPE_CONSTRAINED)
	ret = mdm_sim7000_power_off();
#elif IS_ENABLED(CONFIG_MODEM_SIMCOM_SIM7000_BOOT_TYPE_NORMAL)
	ret = sim7000_pdp_activate();
#else
...
int mdm_sim7000_power_off(void)
{
	...
	mdata.status_flags = 0;
	sim7000_change_state(SIM7000_STATE_OFF);
...
static int modem_init(const struct device *dev)
{
	...
	mdata.dns.recount = CONFIG_MODEM_SIMCOM_SIM7000_DNS_DEFAULT_RECOUNT;
	mdata.dns.timeout = CONFIG_MODEM_SIMCOM_SIM7000_DNS_DEFAULT_TIMEOUT;

	mdata.ftp.read_buffer = NULL;
	mdata.ftp.nread = 0;
	mdata.ftp.state = SIM7000_FTP_CONNECTION_STATE_INITIAL;
	...
	/* Init RSSI query */
	k_work_init_delayable(&mdata.rssi_query_work, sim7000_rssi_query_work);
...
NET_DEVICE_DT_INST_OFFLOAD_DEFINE(0, modem_init, NULL, &mdata, NULL,
				  CONFIG_MODEM_SIMCOM_SIM7000_INIT_PRIORITY, &api_funcs,
				  MDM_MAX_DATA_LENGTH);

NET_SOCKET_OFFLOAD_REGISTER(simcom_sim7000, CONFIG_NET_SOCKETS_OFFLOAD_PRIORITY,
			    NET_AF_UNSPEC, offload_is_supported, sim7000_offload_socket);
