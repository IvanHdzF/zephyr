...
LOG_MODULE_REGISTER(modem_simcom_sim7000_pdp, CONFIG_MODEM_LOG_LEVEL);

#include "sim7000.h"

static const struct setup_cmd band_setup_cmds[] = {
	#if defined(CONFIG_MODEM_SIMCOM_SIM7000_RAT_NB1)
		SETUP_CMD_NOHANDLE("AT+CNMP=38"),
		SETUP_CMD_NOHANDLE("AT+CMNB=2"),
		SETUP_CMD_NOHANDLE("AT+CBANDCFG=\"NB-IOT\"," MDM_LTE_BANDS),
	#endif /* defined(CONFIG_MODEM_SIMCOM_SIM7000_RAT_NB1) */
	#if defined(CONFIG_MODEM_SIMCOM_SIM7000_RAT_M1)
		SETUP_CMD_NOHANDLE("AT+CNMP=38"),
		SETUP_CMD_NOHANDLE("AT+CMNB=1"),
		SETUP_CMD_NOHANDLE("AT+CBANDCFG=\"CAT-M\"," MDM_LTE_BANDS),
	#endif /* defined(CONFIG_MODEM_SIMCOM_SIM7000_RAT_M1) */
	#if defined(CONFIG_MODEM_SIMCOM_SIM7000_RAT_GSM)
		SETUP_CMD_NOHANDLE("AT+CNMP=13"),
	#endif /* defined(CONFIG_MODEM_SIMCOM_SIM7000_RAT_GSM) */
};
 ...
 */
void sim7000_rssi_query_work(struct k_work *work)
{
...
MODEM_CMD_DEFINE(on_cmd_cgatt)
{
	...
	if (cgatt) {
		mdata.status_flags |= SIM7000_STATUS_FLAG_ATTACHED;
	} else {
		mdata.status_flags &= ~SIM7000_STATUS_FLAG_ATTACHED;
	}
...
int sim7000_pdp_activate(void)
{
	...
	while (counter++ < MDM_MAX_CGATT_WAITS &&
		(mdata.status_flags & SIM7000_STATUS_FLAG_ATTACHED) == 0) {
		ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, cgatt_cmd,
					 ARRAY_SIZE(cgatt_cmd), "AT+CGATT?", &mdata.sem_response,
					 MDM_CMD_TIMEOUT);
		if (ret < 0) {
	...
	if (ret < 0 || (mdata.status_flags & SIM7000_STATUS_FLAG_PDP_ACTIVE) == 0) {
	...
	sim7000_change_state(SIM7000_STATE_NETWORKING);
...
int sim7000_pdp_deactivate(void)
{
	...
	if (sim7000_get_state() != SIM7000_STATE_NETWORKING) {
		LOG_WRN("Cannot deactivate pdp context in state: %d", sim7000_get_state());
		goto out;
	}
